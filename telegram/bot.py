...
def lalal(message):
    if message.chat.type == 'private':
        ...
        if message.text == 'KiberNET22':
            bot.send_message(message.chat.id, "Вы видите на экране множество папок. В одной из них следующий ключ – это имя известнейшего программиста современности.")
            bot.send_message(message.chat.id, "Оглянитесь, найдите его, и он подскажет кодовую фразу. Жду и от вас чистосердечного признания в ответ.")
        ...
        #
            ...
            bot.send_message(message.chat.id, "Ваша задача, составить связный текст (он и будет следующей подсказкой) и найти два слова, которые составляют название одной известнейшей транснациональной компании по производству программного обеспечения для различного рода вычислительной техники.")

        elif msg == 'MICROCOMPUTER И SOFTWARE' or msg == 'MICROCOMPUTER SOFTWARE' or msg == 'MICROCOMPUTER, SOFTWARE' or msg == 'SOFTWARE И MICROCOMPUTER' or msg == 'SOFTWARE MICROCOMPUTER' or msg == 'SOFTWARE, MICROCOMPUTER':

            bot.send_message(message.chat.id, "Теперь внимательно прочитайте получившейся текст. Там вас ждёт следующее задание")

        elif msg == 'СОЗДАТЕЛЬ':

            bot.send_message(message.chat.id, "азу на глаз, он расскажет тебе какие два прилагательных являются ключом к победе.")
